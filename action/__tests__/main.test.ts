...
jest.mock('../src/handleValidate', () => {
  const mockResult = (jest.requireActual('./__mocks/mockSarif') as typeof mocks).mockSarifResult
  return {
    __esModule: true,
...
jest.mock('../src/handlePullRequestRun', () => {
  // @ts-ignore
  const { handlePullRequestRun: handlePullRequestRunActual } = jest.requireActual('../src/handlePullRequestRun')
...
describe('main', () => {
  ...
  it('checks out, handles a pr, creates a review with a proper config', async () => {
    ...
    expect(core.setFailed).toHaveBeenCalledWith("Validation failure. CFN Guard found violations.")
